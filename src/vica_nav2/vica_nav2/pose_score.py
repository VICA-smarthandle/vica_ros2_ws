...
#
# 70 -> 65 (2026-09-01 실기 확정, 위 [TARGET] 절차대로): 정답 자리 주변인데
# 71 부터야 통과했고, 주위 5 m 에 벽·기둥이 없는 트인 자리에서는 더 낮게
# 나왔다 — 먼 벽일수록 빔 한 개의 각도 오차가 큰 거리 오차가 되어 점수가
# 깎이기 때문이다.
#
# 65 -> 55 (2026-09-02 사용자 판정). 이 점수는 **평균**이라 지도에 없는 물체가
# 늘어난 만큼 그대로 깎인다. 공공장소에서는 사람·카트·임시 집기가 상시로 있고,
# 그것들은 위치가 틀려서가 아니라 지도에 없어서 0 점이 된다. 실측으로 재 보면
# 깨끗한 조건(지도에 없는 물체 0)에서 30 cm 를 틀어야 65 가 되는데
#
#     0cm 99.5 · 5cm 97.3 · 10cm 96.2 · 15cm 92.6 · 20cm 86.3 · 30cm 64.5
#     (2026-09-02 실측, vica_map_0630 · 좁은 곳/보통/트인 곳 세 자리 평균)
#
# 사람 둘과 카트 하나면 20 %p 가 날아간다(위 주석). 즉 65 를 요구하면 **자세는
# 10 cm 안으로 맞았는데 사람이 많아서 떨어지는** 일이 생긴다.
#
# 낮춘 만큼의 안전은 두 겹이 받는다.
#   1. 라이다 점 겹쳐보기 — 산호빛 점이 벽 위에 얹혔는지 사람이 눈으로 본다.
#      점수가 못 보는 복도 앞뒤 어긋남도 여기서 드러난다.
#   2. AMCL 이 주행하면서 계속 보정한다 — 확정은 출발점일 뿐 최종값이 아니다.
# 그래서 55 는 "이 자세로 출발해도 되는가"의 문턱이지 "정확한가"의 기준이 아니다.
MIN_SCORE = 55.0
WARN_SCORE = 50.0
MIN_BEAMS = 60
MIN_MARGIN = 10.0
