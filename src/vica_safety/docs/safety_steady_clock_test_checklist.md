...
### 6.6 전부하 주행 중 `/motor/can_ok` 관측 (2026-07-28)

`motor_can_timeout_sec`(0.5초)을 넘는 발행 지연이 생기면 장애가 없어도
`motor_can_stale`로 상시 걸쇠가 걸려 주행 자체가 불가능해진다. Nav2 full
stack + nvblox + RealSense가 모두 올라온 상태에서 실주행하며 측정했다.
관측 시작 시점에 nvblox(126초 전), Nav2 controller(61초 전), RealSense,
safety, motor가 모두 기동을 마친 상태였다.

| 항목 | 값 |
| --- | --- |
| 관측 시간 | 175초 (`/safety_state=RUNNING` 20/35 샘플) |
| 장애 구간 | **0회** |
| 발행 주파수 | 29.8 Hz |
| 최대 발행 간격 | **0.109초** (timeout 0.5초 대비 4.6배 여유) |

부하에 따른 변화가 뚜렷하다. 무부하 0.042초(11.9배) → 전부하 주행 0.109초
(4.6배). 0.109초는 Nav2가 `RUNNING`으로 전환하는 순간 한 번 기록된 값이고
이후 130초 동안 더 커지지 않았다. 지속 수준이 아니라 활성화 시 스파이크다.

판단:

- **디바운스 완화는 하지 않는다.** 주행 중 장애가 0회이므로 완화할 근거가
  없고, 완화는 안전을 깎는 방향이다.
- **`motor_can_timeout_sec` 0.5초를 유지한다.** 4.6배 여유가 있다.
- 부하가 더 늘면(추가 센서, 더 무거운 planner) 여유가 다시 절반이 될 수
  있다. 스택 구성이 바뀌면 이 측정을 다시 한다.
