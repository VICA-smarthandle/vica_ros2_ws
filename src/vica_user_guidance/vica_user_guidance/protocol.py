...
# [메모 2026-09-04] 펌웨어는 D11(미장착 자리표시)에서 터치 프레임 AA 56 … 을
# 20Hz 로 올리지만, ROS 쪽 터치 연동은 되돌렸다(사용자 결정: 터치 보류, 햅틱만).
# 초음파 파서는 헤더 둘째 바이트(0x55≠0x56)로 그 프레임을 1바이트씩 버리므로
# 영향이 없다. 터치를 다시 살릴 때는 780f720 을 참고한다.
