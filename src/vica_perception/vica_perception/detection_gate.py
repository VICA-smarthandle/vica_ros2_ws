...
# 상한: 센서가 사람을 "믿을 만하게" 보는 한계다. 2026-09-09 실측(v7 엔진):
#
#     7.0~7.5 m   conf 0.89   stable 100 %
#     7.5~8.0 m   conf 0.82   stable  83 %
#     8.0~8.5 m   conf 0.74   stable   0 %   <- 여기서 무너진다
#     8.5~9.0 m   conf 0.55   stable   0 %
#
# 8 m 를 넘으면 거리 추정이 흔들려 "3초간 0.3 m 미만 이동" 판정이 서지 않는다.
# 그래서 더 올려도 승인이 나오지 않는다 — 이 값은 센서 한계에 맞춘 것이다.
#
# [대가] 종전 4.0 m 의 근거는 "도착까지 10초"였다. 8 m 에서는 실측 19.6 초로
# 그 두 배다(승인 7.77 m -> 1.3 m 도달). 그럼에도 올린 이유는 3 m 앞까지 와야
# 로봇이 반응하는 것이 실제 이용자에게 너무 늦기 때문이다(사용자 판정).
# 시간을 줄이려면 거리가 아니라 속도를 봐야 한다 — 사람 접근 구간은
# PERSON_APPROACH_SPEED_PERCENT(60 %)로 고정 감속되고, 그 위에 모터 knob 상한이
# 또 걸린다(실기 35 % = 0.35 m/s).
